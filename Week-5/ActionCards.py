...
class UnoPlayer:
    ...
    def take_turn(self, deck, pile):
        ...
        if len(matches) > 0:  # can play
            ...
            cardChoice = matches[choice - 1].rank
        else:  # can't play
...
def check_and_take_turn(playerList, deck, pile, currentPlayerNum, lastCard):
    ...
    if lastCard == 'skip':
        ...
        playerList.reverse()    # reverses playerList so the order is reversed
        lastCard = None
        currentPlayerNum = (playerList.index(player) + 1) % len(playerList)
    else:
        lastCard = playerList[currentPlayerNum].take_turn(deck, pile) # takes a turn because nothing special was played on the last turn
