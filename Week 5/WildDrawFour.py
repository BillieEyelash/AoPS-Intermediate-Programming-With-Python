...
class UnoPlayer:
    ...
    def take_turn(self, deck, pile):
        ...
        if len(matches) > 0:  # can play
            ...
            cardChoice = matches[choice - 1]
            cardChoice.set_color_if_wild()
            ...
            if newcard.is_match(topcard): # can be played
                ...
                newcard.set_color_if_wild()
                self.play_card(newcard, pile)
                cardChoiceRank = newcard.rank
...
def check_and_take_turn(playerList, deck, pile, currentPlayerNum, lastCard):
    ...
    if lastCard == 'skip':
        ...
        lastCard = playerList[currentPlayerNum].take_turn(deck, pile)
